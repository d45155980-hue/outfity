...
export default function ProductCard({ product, index = 0 }: ProductCardProps) {
  ...
  const gradientColors = ['from-stone-300', 'to-stone-500'];
      ...
      <Link href={`/products/${product._id}`} className="block">
        <div className="relative aspect-[3/4] bg-gray-100 overflow-hidden rounded-sm">
          {rawImage ? (
            ...
            <div className="w-full h-full bg-gradient-to-br from-stone-300 to-stone-500 flex items-center justify-center text-white/40">
          ...
          <button
            ...
            className="absolute top-2 right-2 w-10 h-10 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center hover:bg-white transition-colors shadow-sm"
          ...
          <div className="absolute inset-x-0 bottom-0 p-1.5 sm:p-3 opacity-0 sm:opacity-0 sm:group-hover:opacity-100 transition-all duration-300 translate-y-0 sm:translate-y-2 sm:group-hover:translate-y-0">
            <button
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
        ...
        <div className="mt-3 sm:mt-4 px-1 sm:px-1">
          <p className="text-xs text-gray-400 uppercase tracking-wider mb-1">
            {product.brand}
          </p>
          <h3 className="text-sm font-medium text-gray-900 mb-1.5 sm:mb-2 line-clamp-2 group-hover:text-gray-600 transition-colors">
            {product.name}
          </h3>
